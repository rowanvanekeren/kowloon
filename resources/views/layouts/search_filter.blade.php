...
<form method="post" action="./{{ $view_name }}" id="searchForm"
        ...
        <div class="filter_collapse_content">
            <div class="col-md-12">
                ...
                @foreach($categories as $category)
                    ...
                    <input type="checkbox" id="{{$category->translation[0]->type}}" value="1"
                           name="chkbx[{{$category->id}}]"
                           @if (old('chkbx.'.$category->id)  == "1") checked @endif />
                    <label for="{{$category->translation[0]->type}}">{{$category->translation[0]->type}}</label>
                @endforeach
            </div>
    ...
    <div class="row search_section">
        <h1 class="search_overlay">Start typing and press enter <img src="{{asset('images/icons/enter.png')}}"> to start</h1>
